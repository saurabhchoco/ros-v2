    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-2xl p-6 w-full max-w-md">
        ...
        <div className="flex justify-end gap-2 mt-4">
          <button onClick={onClose} disabled={submitting} className="px-4 py-2 bg-gray-200 rounded-lg text-sm font-medium hover:bg-gray-300">Cancel</button>
          <button onClick={handleConfirm} disabled={submitting} className="px-4 py-2 bg-red-600 text-white rounded-lg text-sm font-medium hover:bg-red-700 flex items-center gap-2">
...
export default function OrdersList() {
  ...
  const filteredOrders = useMemo(() => {
    let data = [...allOrders];
    if (statusFilter) data = data.filter(o => o.order_status === statusFilter);
    if (search) {
  ...
  useEffect(() => {
    if (filteredOrders.length > 0 && !selectedOrderId) setSelectedOrderId(filteredOrders[0].id);
  }, [filteredOrders, selectedOrderId]);
...
const handleStatusUpdate = async (orderId, newStatus, cancellationReason = null) => {
  ...
  const handleSettle = async (orderId, paymentMethod) => {
    try {
      ...
      const errorMessage = err.response?.data?.message || err.response?.data?.error || 'Settlement failed';
      toast.error(errorMessage);
    ...
    <div className="p-6 max-w-[1600px] mx-auto">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex gap-3">
          <select value={dateRange} onChange={(e) => setDateRange(e.target.value)} className="px-3 py-2 border rounded-lg text-sm bg-white">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-8">
        <div className="bg-white rounded-xl border p-3 shadow-sm">
          <div className="flex items-center gap-2"><div className="p-1.5 bg-indigo-50 rounded-full"><ShoppingBag className="h-4 w-4 text-indigo-600" /></div><div><p className="text-xs text-gray-500">Orders Today</p><p className="text-xl font-bold">{stats.total}</p></div></div>
        </div>
        <div className="bg-white rounded-xl border p-3 shadow-sm">
          <div className="flex items-center gap-2"><div className="p-1.5 bg-orange-50 rounded-full"><Clock className="h-4 w-4 text-orange-600" /></div><div><p className="text-xs text-gray-500">Pending</p><p className="text-xl font-bold text-orange-600">{stats.pending}</p></div></div>
        </div>
        <div className="bg-white rounded-xl border p-3 shadow-sm">
          <div className="flex items-center gap-2"><div className="p-1.5 bg-green-50 rounded-full"><CheckCircle className="h-4 w-4 text-green-600" /></div><div><p className="text-xs text-gray-500">Ready</p><p className="text-xl font-bold text-green-600">{stats.ready}</p></div></div>
        </div>
        <div className="bg-white rounded-xl border p-3 shadow-sm">
          <div className="flex items-center gap-2"><div className="p-1.5 bg-red-50 rounded-full"><XCircle className="h-4 w-4 text-red-600" /></div><div><p className="text-xs text-gray-500">Cancelled</p><p className="text-xl font-bold text-red-600">{stats.cancelled}</p></div></div>
        </div>
        <div className="bg-white rounded-xl border p-3 shadow-sm">
          <div className="flex items-center gap-2"><div className="p-1.5 bg-yellow-50 rounded-full"><Wallet className="h-4 w-4 text-yellow-600" /></div><div><p className="text-xs text-gray-500">Settlement Pending</p><p className="text-xl font-bold text-yellow-600">{stats.settlementPending}</p></div></div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border p-4 mb-6 flex flex-wrap gap-3 items-end">
        <div className="flex-1 min-w-[200px]">
          ...
          <div className="relative">
            ...
            <input type="text" placeholder="Order #, customer, table, token..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-9 pr-3 py-2 border rounded-lg text-sm" />
          </div>
        </div>
        <div className="w-48">
            ...
            <div className="divide-y divide-gray-100">
              {filteredOrders.length === 0 ? (
                <div className="py-16 px-4 text-center">
                  <div className="bg-gray-100 rounded-full w-16 h-16 flex items-center justify-center mx-auto mb-4"><Coffee className="h-8 w-8 text-gray-400" /></div>
                  <h3 className="text-lg font-medium text-gray-900 mb-1">No Orders Yet</h3>
                  <p className="text-sm text-gray-500 max-w-sm mx-auto">Orders created from Captain, QR ordering, and POS will appear here automatically.</p>
                </div>
              ) : (
                filteredOrders.map(order => {
                  ...
                  return (
                    <div key={order.id} className={`p-4 hover:bg-gray-50 cursor-pointer transition ${selectedOrderId === order.id ? 'bg-indigo-50 border-l-4 border-indigo-500' : 'border-l-4 border-transparent'}`} onClick={() => setSelectedOrderId(order.id)}>
                      <div className="flex justify-between items-start">
                        <div className="flex-1">
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {ageText && <span className={`text-xs px-2 py-0.5 rounded-full ${isOverdue ? 'bg-red-100 text-red-800 animate-pulse' : 'bg-gray-100 text-gray-600'}`}>{ageText}</span>}
                          </div>
                          <div className="text-xs text-gray-500 mt-1">{displayCustomer} • {formatTime(order.created_at)}</div>
                        </div>
            ...
            <div className="bg-white rounded-xl border shadow-sm sticky top-6">
                ...
                <div className="flex justify-between items-start">
                  ...
                  <div>
                    {selectedOrder.order_status === 'COMPLETED' && selectedOrder.payment_status !== 'PAID' && (
                      <span className="inline-flex items-center gap-1 text-xs bg-yellow-100 text-yellow-800 px-2 py-1 rounded-full"><AlertCircle className="h-3 w-3" /> Settlement Pending</span>
                    )}
                    {selectedOrder.payment_status === 'PAID' && (
                      <span className="inline-flex items-center gap-1 text-xs bg-green-100 text-green-800 px-2 py-1 rounded-full"><CheckCircle className="h-3 w-3" /> Settled</span>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className="p-4 space-y-4">
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between"><span className="text-gray-500">Source</span><span className="font-medium">{orderSourceLabel[selectedOrder.order_source] || selectedOrder.order_source}</span></div>
                  ...
                  <div>
                    ...
                    <div className="space-y-1 text-sm">
                      {selectedOrder.items.map((item, idx) => (
                        <div key={idx} className="flex justify-between"><span>{item.quantity}x {item.itemName}</span><span>₹{Number(item.lineTotal).toFixed(0)}</span></div>
                      ))}
                    </div>
                  </div>
                )}
                ...
                <div className="pt-2 border-t space-y-2">
                  {selectedOrder.order_status === 'NEW' && (
                    <>
                      ...
                      >
                        {updatingOrderId === selectedOrder.id ? <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white mx-auto"></div> : 'Start Preparing'}
                      </Button>
                      <Button
                        onClick={() => {
                  ...
                  {selectedOrder.order_status === 'PREPARING' && (
                    <Button onClick={() => handleStatusUpdate(selectedOrder.id, 'READY')} disabled={updatingOrderId === selectedOrder.id} className="w-full disabled:opacity-50 disabled:cursor-not-allowed">
                      {updatingOrderId === selectedOrder.id ? <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white mx-auto"></div> : 'Mark Ready'}
                    </Button>
                  )}
                  {selectedOrder.order_status === 'READY' && (
                    <button
                      ...
                      className="w-full py-2 rounded-lg text-white font-medium bg-green-600 hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                  ...
                  {selectedOrder.order_status === 'COMPLETED' && selectedOrder.payment_status !== 'PAID' && (
                    <div className="space-y-2">
                      <Button onClick={() => handleSettle(selectedOrder.id, 'CASH')} className="w-full">Settle Cash</Button>
                      <Button onClick={() => handleSettle(selectedOrder.id, 'UPI')} className="w-full">Settle UPI</Button>
                      <Button onClick={() => handleSettle(selectedOrder.id, 'CARD')} className="w-full">Settle Card</Button>
                    </div>
                  )}
                </div>
              </div>
